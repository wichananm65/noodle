<template>
    <div>
            ...
            <div class="input-group mb-3">
                <form>
                    <input type="text" v-model="search" placeholder="Search" class="form-control"
                        ...
                        <a v-on:click.prevent="setProduction(' ')" href="#"><button class="btn btn-outline-secondary"
                            type="button" id="button-addon2">Clear</button></a>
                </form>
            </div>
            <div>
        ...
        <div id="noodle-list-bottom">
            <div class="noodle-load-finished"
                v-if="noodles.length === filteredNoodles.length && filteredNoodles.length > 0 && loading === false">
                โหลดข้อมูลครบแล้ว
            </div>
        </div>
...
export default {
    ...
    async created() {
        // Fetch all noodles when the component is added
        this.loading = true;
        this.results = (await NoodlesService.index()).data;
        this.noodles = this.results; // Set noodles to show all by default
        this.production = [...new Set(this.results.map(noodle => noodle.production))]; // Unique categories
        this.loading = false;
    },
    updated() {
        let sens = document.querySelector('#noodle-list-bottom')
        pageWatcher = ScrollMonitor.add(sens)
    ...
    methods: {
        ...
        async refreshData() {
            this.results = (await NoodlesService.index()).data;
            this.noodles = this.results; // Refresh noodles to show all
        }
    }
}
</script>
